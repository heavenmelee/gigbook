...
export default function MusicianChatScreen() {
  ...
  const filteredConversations = conversations?.filter((conv) => {
    const customerName = conv.otherUser?.name || "";
    const bookingEvent = conv.booking?.venueName || "";
    const query = searchQuery.toLowerCase();
    return customerName.toLowerCase().includes(query) || bookingEvent.toLowerCase().includes(query);
  }) || [];
      ...
      {isLoading ? (
        ...
        <View style={s.emptyContainer}>
          ...
          <Text style={[s.emptySubtitle, { color: colors.muted }]}>
            Customers will message you when they book
          </Text>
        </View>
      ) : (
        <FlatList
          ...
          renderItem={({ item: conv }) => (
            ...
            >
              <View style={s.cardContent}>
                ...
                <View style={s.messageInfo}>
                  ...
                  <Text
                    style={[
                      s.lastMessage,
                      {
                        color: (conv.unreadByMusician || 0) > 0 ? colors.foreground : colors.muted,
                        fontWeight: (conv.unreadByMusician || 0) > 0 ? "600" : "400",
                      },
                    ]}
                    numberOfLines={1}
                  >
                ...
                {/* Unread Badge */}
                {(conv.unreadByMusician || 0) > 0 && (
                  <View style={[s.unreadBadge, { backgroundColor: colors.primary }]}>
                    <Text style={s.unreadBadgeText}>{conv.unreadByMusician}</Text>
                  </View>
                )}
              </View>
            </TouchableOpacity>
